...
class U1652DatasetTrain(Dataset):
    ...
    def shuffle(self, ):
            ...
            pairs_epoch = set()  # 创捷一个集合变量，元素无序且不可重复
            idx_batch = set()
            ...
            while True:
                ...
                if len(pair_pool) > 0:
                    pair = pair_pool.pop(0) # Python 列表中的一个方法，用于移除并返回指定索引处的元素
                    ...
                    else: # 意味着卫星图像已经存在对应的了
                        # if pair fits not in batch and is not already used in epoch -> back to pool
                        if pair not in pairs_epoch: # 判断对应项是否重复，按道理来讲应该不存在重复项的，应该一直是True
                            pair_pool.append(pair) # 返回配对池中
                            
                        break_counter += 1
                        
                    if break_counter >= 512: # 连续512次都没
                        break
                   
                else:
                    break

                if len(current_batch) >= self.shuffle_batch_size: # 每达到设置的batch_size时，就重置，并重新添加
            ...
            self.samples = batches # 最终保证每个batch中都没有重复的卫星图
